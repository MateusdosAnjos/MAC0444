		...
		\item[\textbf{2 -}]
			...
			\begin{itemize}
				...
				\item[\textbf{C) }]
					...
					membros do clube alpino, não é possível provar que (sempre) existe um membro do 
					clube 
					alpino que é alpinista mas não é esquiador. (A situação do contra-exemplo mostra
					um caso em que não existe um membro do clube alpino que é alpinista mas não
					é esquiador).
			\newpage
			\item[\textbf{D) }]
				\hfill\newline
				Primeiro organizaremos nossas cláusulas na Forma Normal Conjuntiva
				para podermos utilizar a resolução.
				\begin{center}
					Base de Conhecimento (KB):\\ \hfill \\
					\begin{tabular}{ll}
						1. & $[MembroClubeAlpino(Tony)]$\\
